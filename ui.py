...
class gamelogic:
    ...
    def makeGuess(self, guess):
        prettyGuess = list() # With colorama.Fore
        for index, letter in enumerate(guess):
            if letter == self.CWord[index]: prettyGuess.append(Green(letter))
            elif letter not in self.CWord: prettyGuess.append(Red(letter))
            else:
                ...
                if index in YellowGuess:
                    prettyGuess.append(Yellow(letter))
                else:
                    prettyGuess.append(Red(letter))
                # BOOTS CWORD 2 # [3]
                # IMOoO GUESS 1
                # 4 [4]
